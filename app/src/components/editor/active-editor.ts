// The active editing surface (the block surface, or the raw source view)
// registers its flush here so app-lifecycle code can force its pending,
// debounced snapshot into the store synchronously — before an explicit save
// (⌘S) reads the store, and before the app quits.
//
...
// single slot suffices. The block surface's command dispatch (the ⌘⇧P Insert
// group, link affordance) goes through active-surface.ts.
