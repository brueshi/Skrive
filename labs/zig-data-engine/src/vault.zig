...
pub fn load(
    ...
    while (try walker.next(io)) |entry| {
        ...
        // The document's name, indexed as a block of its own so it competes
        // in ranking like anything else: the filename with its separators
        // opened out, plus the first heading, which is how these documents
        // actually announce themselves.
        {
            var title: std.ArrayList(u8) = .empty;
            try appendStemWords(fa, &title, entry.basename);
            for (scanned_blocks) |b| {
                if (b.kind != .heading) continue;
                try title.append(fa, ' ');
                try title.appendSlice(fa, std.mem.trim(u8, std.mem.trimStart(u8, std.mem.trim(u8, b.text, " \t\r\n"), "#"), " \t"));
                break;
            }

            const h = try markdown.harvest(fa, .{ .kind = .title, .text = title.items });
            try idx.putBlock(next_block, .{ .doc = doc, .kind = .title }, h.tokens);
            next_block += 1;
            stats.blocks += 1;
        }
...
/// `chrome-navigation-model.md` becomes `chrome navigation model`. Separators
/// are what a filename uses instead of spaces, so opening them out is what
/// makes the name searchable as words rather than as one opaque token.
fn appendStemWords(
    gpa: std.mem.Allocator,
