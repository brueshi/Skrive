// The table hover chrome: the gutters that let a writer add, select, resize,
// and reorder rows and columns by pointing at the table. The chrome lives in a
// layer the host provides OUTSIDE its editable, positioned in the scroller's
// content coordinate space so it rides scroll with no listener. It is built
// only while a table is hovered or focused, is O(rows + cols), and rebuilds
// from the host's structural-change signal, never from the keystroke path.
//
// Geometry is measured, never modeled: the slot positions come from the real
// cell rects. The measurement step is isolated from the arithmetic in
// geometry.ts, which is pure and tested without layout.
//
// Pointer rules this file encodes for WKWebView: a toggle binds to click, never
// pointerup (dropped on a motionless press); a real drag uses pointer events;
...
export function measureTable(
  ...
  const toContent = (el: Element): Box => contentBox(el.getBoundingClientRect(), hostRect, scrollLeft, scrollTop);
...
/**
 * What the chrome needs from the editor it lives in. Tables are addressed by the
 * host's id for the table element, and every mutation is an intent the host
 * applies (and lands the caret for). The per-table `TableHost` of the contract
 * arrives with the stateful surface; this is the seam the shipped painter has.
 */
export type TableChromeHost = {
  /** The editable holding the tables. */
  surface: HTMLElement;
  /** The scrolling container; slots sit in its content coordinate space. */
  scroller: HTMLElement;
  /** The overlay the slots are appended to; outside the editable. */
  layer: HTMLElement;
...
export function attachTableChrome(host: TableChromeHost): TableChromeHandle {
  const { surface, scroller, layer } = host;
  ...
  const renderSlot = (blockId: string, table: HTMLTableElement, slot: GutterSlot): void => {
    ...
    el.addEventListener('click', (e) => {
      ...
      switch (slot.kind) {
        case 'col-append':
          host.apply(blockId, { type: 'insert-column', index });
          break;
        case 'row-append':
  ...
  const beginHandleReorder = (e: PointerEvent, blockId: string, table: HTMLTableElement, slot: GutterSlot, el: HTMLElement): void => {
    ...
    const cleanup = (): void => {
      ...
      if (moved) {
        ...
        indicator?.remove();
        for (const c of table.querySelectorAll(`[${CELL_DRAGGING_ATTR}]`)) c.removeAttribute(CELL_DRAGGING_ATTR);
      }
    };
    const onMove = (ev: PointerEvent): void => {
      if (!moved) {
        ...
        const sliceKey = kind === 'col' ? CELL_COL_ATTR : CELL_ROW_ATTR;
        for (const c of table.querySelectorAll(`[${sliceKey}="${from}"]`)) c.setAttribute(CELL_DRAGGING_ATTR, '');
      ...
      const pos =
        kind === 'col'
          ? ev.clientX - scrollerRect.left + scroller.scrollLeft
          : ev.clientY - scrollerRect.top + scroller.scrollTop;
    ...
    const onUp = (): void => {
      ...
      if (didMove) {
        suppressClick = true;
        host.apply(blockId, kind === 'col' ? { type: 'move-column', from, to } : { type: 'move-row', from, to });
      }
    };
  ...
  const beginColumnResize = (e: PointerEvent, blockId: string, table: HTMLTableElement, slot: GutterSlot, el: HTMLElement): void => {
    ...
    const end = (): void => {
      ...
      if (moved) host.apply(blockId, { type: 'set-widths', widths: normalizeWidths(current) });
    };
  ...
  const layerSelectedTable = (): HTMLTableElement | null =>
    selection ? host.findTable(selection.tableId) : null;
  ...
  // keystroke path.
  const unsubscribe = host.onStructureChange(() => {
