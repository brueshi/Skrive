...
The editor toolbar has a **permanent set that never grows**: `[ Turn into ▾ ] | B  I  ⌍code⌎ | [ Insert ▾ ]`. A new feature may add a slash entry, an Insert-catalog row, a palette command, a keyboard shortcut, per-block chrome, or a settings control — but it may **never add a toolbar button**. Changing the permanent set requires editing the grammar doc first; that friction is deliberate.
